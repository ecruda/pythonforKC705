...
def main():
    ...
    with open("GBS20_SEU_RESULT_log10.txt","w") as infile:
        while True:
            
            if(datetime.datetime.now() - lasttime > datetime.timedelta(seconds=1)):
                lasttime = datetime.datetime.now()
                print(lasttime, hex(cmd_interpret.read_status_reg(8)), hex(cmd_interpret.read_status_reg(6)), hex((cmd_interpret.read_status_reg(4) << 48) + (cmd_interpret.read_status_reg(2) << 32) + (cmd_interpret.read_status_reg(2) << 16) +  cmd_interpret.read_status_reg(1)))
